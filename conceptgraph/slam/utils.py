...
def get_bounding_box(cfg, pcd):
    if ("accurate" in cfg.spatial_sim_type or
            "overlap" in cfg.spatial_sim_type) and len(pcd.points) >= 4:
        try:
...
def merge_obj2_into_obj1(cfg, obj1, obj2, run_dbscan=True):
    ...
    obj1['text_ft'] = (obj1['text_ft'] * n_obj1_det +
                       obj2['text_ft'] * n_obj2_det) / (n_obj1_det + n_obj2_det)
    ...
    for i in range(n):
        for j in range(n):
            if i != j:  # Skip diagonal elements
                ...
                overlap = (D < cfg.downsample_voxel_size**2
...
def compute_overlap_matrix_2set(cfg, objects_map: MapObjectList,
                                objects_new: DetectionList) -> np.ndarray:
    '''
...
def merge_overlap_objects(cfg, objects: MapObjectList,
    ...
    for i, j, ratio in zip(x, y, overlap_ratio):
        visual_sim = F.cosine_similarity(to_tensor(objects[i]['clip_ft']),
                                         to_tensor(objects[j]['clip_ft']),
                                         dim=0)
        text_sim = F.cosine_similarity(to_tensor(objects[i]['text_ft']),
        ...
        if ratio > cfg.merge_overlap_thresh:
            if visual_sim > cfg.merge_visual_sim_thresh and \
                ...
                if kept_objects[j]:
                    ...
                    objects[j] = merge_obj2_into_obj1(cfg,
                                                      objects[j],
                                                      objects[i],
                                                      run_dbscan=True)
...
def filter_objects(cfg, objects: MapObjectList):
    ...
    objects_to_keep = []
    for obj in objects:
        if len(obj['pcd'].points) >= cfg.obj_min_points and obj[
                'num_detections'] >= cfg.obj_min_detections:
