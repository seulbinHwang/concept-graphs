...
SCENE_NAME=room0
THRESHOLD=1.2
python -m conceptgraph.slam.cfslam_pipeline_batch \
...
def compute_match_batch(cfg, spatial_sim: torch.Tensor,
    ...
    if cfg.match_method == "sim_sum":
        sims = (1 + cfg.phys_bias) * spatial_sim + (
            1 - cfg.phys_bias) * visual_sim  # (M, N)
...
def main(cfg: DictConfig):
    ...
    for idx in trange(len(dataset)):
        ...
        spatial_sim = compute_spatial_similarities(cfg, fg_detection_list,
                                                   objects)
        visual_sim = compute_visual_similarities(cfg, fg_detection_list,
                                                 objects)
        ...
        objects = merge_detections_to_objects(cfg, fg_detection_list, objects,
                                              agg_sim)
        ...
        if cfg.save_objects_all_frames:
            ...
            if not cfg.skip_bg:
                ...
                bg_objects_to_save = prepare_objects_save_vis(
                    bg_objects_to_save)
            else:
    ...
    if cfg.save_pcd:
        ...
        with gzip.open(pcd_save_path, "wb") as f:
            pickle.dump(results, f)
        print(
            f"Saved full point cloud after post-processing to {pcd_save_path}")
    ...
    if cfg.vis_render:
        ...
        video_save_path = (cfg.dataset_root / cfg.scene_id /
                           ("objects_mapping-%s-%s.mp4" %
                            (cfg.gsa_variant, cfg.save_suffix)))
