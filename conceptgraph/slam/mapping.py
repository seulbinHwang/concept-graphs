...
def compute_spatial_similarities(cfg, detection_list: DetectionList,
    ...
    if cfg.spatial_sim_type == "iou":
        ...
        raise ValueError(
            f"Invalid spatial similarity type: {cfg.spatial_sim_type}")

    return spatial_sim


def compute_visual_similarities(cfg, detection_list: DetectionList,
                                objects: MapObjectList) -> torch.Tensor:
    '''
...
def aggregate_similarities(cfg, spatial_sim: torch.Tensor,
...
def merge_detections_to_objects(cfg, detection_list: DetectionList,
    ...
    for i in range(agg_sim.shape[0]):
        ...
        # Merge with most similar existing object
            ...
            merged_obj = merge_obj2_into_obj1(cfg,
                                              matched_obj,
                                              matched_det,
                                              run_dbscan=False)
