...
class OnlineObjectRenderer():
    ...
    def __init__(self,
        ...
        if base_objects is not None:
            ...
            for i in range(self.n_base_objects):
                base_pcds_vis[i] = base_pcds_vis[i].voxel_down_sample(
                    voxel_size=0.08)
                if gray_map:
    ...
    def filter_base_by_mask(self, mask: Iterable[bool]):
        ...
        self.base_pcds_vis = [
            pcd for pcd, m in zip(self.base_pcds_vis, mask) if m
        ]
    ...
    def step(
        ...
        if est_pose is not None:
            ...
            est_camera_frustum = better_camera_frustum(est_pose,
                                                       image.height,
                                                       image.width,
                                                       scale=0.5,
                                                       color=[1., 0, 0])
            ...
            if len(self.est_traj) > 1:
                est_traj_lineset = poses2lineset(np.stack(self.est_traj),
                                                 color=[1., 0, 0])
                self.vis.add_geometry(est_traj_lineset)

        if gt_pose is not None:
            self.gt_traj.append(gt_pose)
            gt_camera_frustum = better_camera_frustum(gt_pose,
                                                      image.height,
                                                      image.width,
                                                      scale=0.5,
                                                      color=[0, 1., 0])
            ...
            if len(self.gt_traj) > 1:
                gt_traj_lineset = poses2lineset(np.stack(self.gt_traj),
                                                color=[0, 1., 0])
...
def vis_result_fast_on_depth(
    ...
    annotated_image = mask_annotator.annotate(scene=depth_image.copy(),
...
def old_filter_detections(
    ...
    for index, (confidence, class_id, xyxy, mask) in enumerate(
            zip(detections.confidence, detections.class_id, detections.xyxy,
                detections.mask)):
        if confidence >= confidence_threshold:
            detections_combined.append(
                (confidence, class_id, xyxy, mask, index))
    ...
    for det_idx, (confidence, class_id, xyxy, mask,
                  original_index) in enumerate(detections_combined):
...
class CustomBoxAnnotator(sv.BoxAnnotator):
    ...
    def find_label_position(self, x1, y1, x2, y2, text_width, text_height,
        ...
        for pos in positions_list:
            collision_flag = self.is_area_occupied(pos['x1'], pos['y1'],
                                                   pos['x2'], pos['y2'])
            logging.debug(
    ...
    def annotate(
        ...
        for i, (x1, y1, x2, y2) in enumerate(detections.xyxy.astype(int)):
            ...
            text = labels[i] if labels is not None and len(labels) == len(
                detections) else str(class_id)
            color = self.color.by_idx(class_id) if isinstance(
                self.color, ColorPalette) else self.color
            cv2.rectangle(scene, (x1, y1), (x2, y2),
                          color=color.as_bgr(),
        ...
        for i, (x1, y1, x2, y2) in enumerate(detections.xyxy.astype(int)):
            ...
            label_x1, label_y1, label_x2, label_y2, pos_desc = self.find_label_position(
                x1, y1, x2, y2, text_width, text_height, scene.shape[0],
                scene.shape[1])

            self.mark_area_as_occupied(label_x1, label_y1, label_x2, label_y2)
            cv2.rectangle(scene, (label_x1, label_y1), (label_x2, label_y2),
                          color=color.as_bgr(),
                          thickness=cv2.FILLED)

            cv2.putText(scene, text, (int(label_x1) + self.text_padding,
                                      int(label_y2) - self.text_padding), font,
                        self.text_scale, self.text_color.as_rgb(),
...
def vis_result_slow_caption(image, masks, boxes_filt, pred_phrases, caption,
                            text_prompt):
    '''
...
def create_camera_frustum(camera_pose,
                          width=1,
                          height=1,
                          z_near=0.5,
                          z_far=1,
    ...
    points_transformed = camera_pose[:3, :3] @ (K @ points.T) + camera_pose[:3,
                                                                            3:4]
...
def better_camera_frustum(camera_pose,
                          img_h,
                          img_w,
                          scale=3.0,
                          color=[0, 0, 1]):
