    ...
    def set_use_rerun(self, config_use_rerun):
        ...
        if self._config_use_rerun and self._rerun is None:
            try:
                ...
                logging.info(
                    "rerun is not installed. Not using rerun for logging.")
        else:
            logging.info(
...
특히, 카메라의 현재 위치와 자세(orientation)를 기록하고,
    ...
    orr.log(
        "world/camera",
        orr.Transform3D(translation=translation,
                        rotation=quaternion,
                        from_parent=False))
...
def orr_log_rgb_image(color_path):
    ...
    orr.log("world/camera/rgb_image_encoded",
...
def orr_log_vlm_image(vlm_image_path, label=""):
    if os.path.exists(vlm_image_path):
        orr.log(f"world/camera/vlm_image_{label}",
...
def orr_log_edges(objects, map_edges, obj_classes):
    ...
    for map_edge_tuple in map_edges.edges_by_index.items():
        ...
        obj_2_color = obj_classes.get_class_color(
            objects[obj2_idx]['class_name'])
        orr.log(
            base_entity_path + f"/edges_no_labels" +
        ...
        orr.log(
            base_entity_path + f"/edges_w_num_det_labels" +
            f"/{edge_label_by_curr_num}",
            orr.LineStrips3D(
                endpoints,
                labels=[f"{num_dets}"],
        ...
        orr.log(
            base_entity_path + f"/edges_w_rel_type_labels" +
            f"/{edge_label_by_curr_num}",
            orr.LineStrips3D(
                endpoints,
                labels=[f"{rel_type}"],
        ...
        orr.log(
            base_entity_path + f"/edges_w_full_labels" +
            f"/{edge_label_by_curr_num}",
            orr.LineStrips3D(
                endpoints,
                labels=[f"{full_label}"],
