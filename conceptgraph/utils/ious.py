...
def compute_giou_batch(bbox1: torch.Tensor,
    ...
    inter_vol = torch.prod(torch.clamp(inter_max - inter_min, min=0),
                           dim=2)  # Shape: (M, N)
    enclosing_vol = torch.prod(enclosing_max - enclosing_min,
                               dim=2)  # Shape: (M, N)
...
def expand_3d_box(bbox: torch.Tensor, eps=0.02) -> torch.Tensor:
    ...
    a = torch.linalg.vector_norm(va, ord=2, dim=1,
                                 keepdim=True)  # shape: (N, 1)
    b = torch.linalg.vector_norm(vb, ord=2, dim=1,
                                 keepdim=True)  # shape: (N, 1)
    c = torch.linalg.vector_norm(vc, ord=2, dim=1,
                                 keepdim=True)  # shape: (N, 1)
...
def compute_enclosing_vol(bbox1: torch.Tensor,
                          bbox2: torch.Tensor) -> torch.Tensor:
    '''
    ...
    for i in range(bbox1.shape[0]):
        for j in range(bbox2.shape[0]):
            ...
            bbox_points_union = torch.cat([bbox1[i], bbox2[j]],
                                          dim=0)  # (16, 3)
            pcd_union.points = o3d.utility.Vector3dVector(
                bbox_points_union.cpu().numpy())
            enclosing_mesh, _ = pcd_union.compute_convex_hull(
                joggle_inputs=True)
            try:
...
def compute_enclosing_vol_fast(bbox1: torch.Tensor,
                               bbox2: torch.Tensor) -> torch.Tensor:
    '''
    Compute the enclosing volume between every pair of boxes in bbox1 and bbox2.
...
def compute_3d_giou_accurate_batch(bbox1: torch.Tensor,
                                   bbox2: torch.Tensor) -> torch.Tensor:
    '''
...
def compute_3d_contain_ratio_accurate_batch(
        bbox1: torch.Tensor, bbox2: torch.Tensor) -> torch.Tensor:
    '''
    ...
    inter_vol, iou = ops.box3d_overlap(
        bbox1[:, [0, 2, 5, 3, 1, 7, 4, 6]].float(),
        bbox2[:, [0, 2, 5, 3, 1, 7, 4, 6]].float())  # (M, N), (M, N)
...
def compute_2d_box_contained_batch(bbox: torch.Tensor,
                                   thresh: float = 0.95) -> torch.Tensor:
    '''
    ...
    inter = (rb - lt).clamp(
        min=0)  # intersection sizes (dx, dy), if no overlap, clamp to zero
...
def mask_subtract_contained(xyxy: np.ndarray,
                            mask: np.ndarray,
                            th1=0.8,
                            th2=0.7):
    """
위 함수는
    객체 감지 결과에서 마스크(mask) 간의 포함 관계를 확인하고,
    큰 객체(예: 소파) 안에 포함된 작은 객체(예: 베개)의 마스크를 분리하는 작업을 수행
    이 작업은 객체들이 서로 포함 관계에 있을 때, 잘못된 객체 분할을 방지하기 위해 사용됩니다.
    예를 들어, 소파 위에 있는 베개가 소파와 같은 객체로 인식되지 않도록 하기 위해,
        베개의 마스크가 소파의 마스크에서 제거됩니다.

### 주요 기능과 역할:

1. **입력 데이터**:
   - `xyxy`: 각 객체의 경계 상자 좌표를 나타내는 배열입니다. 크기는 `(N, 4)`로, `N`은 객체의 수를 나타내며, 각 객체는 `(x1, y1, x2, y2)` 형식으로 경계 상자 좌표를 가집니다.
   - `mask`: 각 객체의 마스크를 나타내는 이진 마스크 배열입니다. 크기는 `(N, H, W)`로, `N`은 객체의 수, `H`와 `W`는 이미지의 높이와 너비입니다.
   - `th1` 및 `th2`: 경계 상자 포함 관계를 판단하는 데 사용되는 임계값입니다. `th1`과 `th2`는 각각 교차 영역(intersection area)이 객체의 전체 영역에 대해 어느 정도 비율을 차지하는지를 나타냅니다.

2. **경계 상자 영역 계산**:
   - 각 경계 상자(`xyxy`)의 면적을 계산합니다. 이는 나중에 교차 영역(intersection area)을 계산할 때 사용됩니다.

3. **교차 영역(intersection area) 계산**:
   - 각 경계 상자 간의 교차 영역을 계산합니다. 이를 위해 두 경계 상자의 좌측 상단(left-top)과 우측 하단(right-bottom) 좌표를 비교하여 교차 영역을 구합니다.
   - 교차 영역의 크기를 계산하여, 이 영역이 각 경계 상자에서 차지하는 비율을 구합니다 (`inter_over_box1`과 `inter_over_box2`).

4. **포함 관계 판단**:
   - 각 경계 상자 쌍에 대해 교차 영역 비율을 비교하여 포함 관계를 판단합니다.
   - 교차 영역이 하나의 경계 상자에 대해 `th2`보다 작고, 다른 경계 상자에 대해 `th1`보다 크다면, 두 경계 상자 간의 포함 관계가 성립됩니다.
   - 이러한 포함 관계를 만족하는 경계 상자 쌍의 인덱스를 `contained_idx`로 저장합니다.

5. **마스크에서 포함된 객체 제거**:
   - 포함된 작은 객체의 마스크를 큰 객체의 마스크에서 제거합니다. 이를 통해 서로 포함된 객체들이 하나의 객체로 인식되는 것을 방지합니다.

6. **결과 반환**:
   - 포함 관계가 처리된 새로운 마스크 배열 `mask_sub`를 반환합니다. 이 마스크 배열은 객체 간의 잘못된 포함 관계가 제거된 상태입니다.

### 요약:
이 함수는 객체 감지 결과에서 서로 포함 관계에 있는 객체들의 마스크를 분리하여, 객체 간의 혼동을 줄이는 역할을 합니다. 이를 통해 객체가 더 정확하게 구분될 수 있도록 하며, 특히 서로 겹쳐 있는 객체들을 올바르게 분리하는 데 중요한 역할을 합니다.
    """
    ...
    # Compute intersection boxes
    lt = np.maximum(xyxy[:, None, :2],
                    xyxy[None, :, :2])  # left-top points (N, N, 2)
    rb = np.minimum(xyxy[:, None, 2:],
                    xyxy[None, :, 2:])  # right-bottom points (N, N, 2)
