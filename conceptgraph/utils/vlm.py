...
### 1. **주요 역할**
- **여러 캡션을 입력**으로 받아, **중복 요소를 제거**하고 **하나의 통합된 캡션**을 생성
    - GPT 모델을 사용하여 캡션을 자동으로 통합
...
2. **시스템 프롬프트 설정**:
   - **GPT-4V**가 해야 할 작업을 정의하는 **프롬프트**를 설정
...
5. **오류 처리 및 결과 반환**:
   - API 호출 중 오류가 발생할 경우, 빈 리스트를 반환하도록 예외 처리가 되어 있음
   - 함수는 최종적으로 각 객체에 대한 **ID, 이름, 캡션**을 포함한 리스트를 반환
