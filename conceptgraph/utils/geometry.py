...
def transform_points_batch(poses: torch.Tensor,
...
def project_points_camera2plane(points: torch.Tensor,
    ...
    points_coord = points_proj[:2, :] / points_proj[2, :].unsqueeze(
        0)  # divide x and y by z

    return points_coord.t(), points_depth  # transpose back to (N, 2)


def project_points_world2plane(points: torch.Tensor, poses: torch.Tensor,
                               K: torch.Tensor) -> torch.Tensor:
...
def check_proj_points(
    ...
    proj_xy_within = torch.stack([
        0 <= proj_xy[:, 0], proj_xy[:, 0] < depth_tensor.shape[1], 0
        <= proj_xy[:, 1], proj_xy[:, 1] < depth_tensor.shape[0]
    ],
                                 dim=-1)  # (N, 2)
    ...
    if proj_within.any():
        depth_within = depth_tensor[proj_xy[proj_within][:, 1].long(),
                                    proj_xy[proj_within][:, 0].long()]  # (N, )
        proj_z_within = proj_z[proj_within]  # (N, )

        proj_front[
            proj_within] = proj_z_within < depth_within - depth_margin  # (N, )
        proj_align[proj_within] = torch.abs(
            proj_z_within - depth_within) <= depth_margin  # (N, )
        proj_behind[
            proj_within] = proj_z_within > depth_within + depth_margin  # (N, )
