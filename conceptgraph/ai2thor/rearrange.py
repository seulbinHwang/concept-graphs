...
def rearrange_objects(
    ...
    moveable_objects = [
        o for o in objects_init if o["moveable"] and not o["pickupable"]
    ]

    n_move_pickupable = int(
        len(pickupable_objects_init) * pickupable_move_ratio)
    n_move_moveable = int(len(moveable_objects) * moveable_move_ratio)

    print(
        f"Try moving {n_move_pickupable}/{len(pickupable_objects_init)} pickupable objects and {n_move_moveable}/{len(moveable_objects)} moveable objects"
    )
    ...
    while n_moved_moveable < n_move_moveable:
        ...
        if obj_id in receptacle2obj:
            ...
            for o_id in receptacle2obj[obj_id]:
                o = next(_ for _ in controller.last_event.metadata["objects"]
                         if _['objectId'] == o_id)
                ...
                if not event.metadata['lastActionSuccess']:
                    ...
                    print(
                        f"Failed to teleport {o['objectId']} contained by {obj_id}"
                    )
    ...
    if len(pickupable_objects_init) > 0:
        ...
        event = controller.step(
            ...
            excludedObjectIds=[
                o["objectId"] for o in pickupable_objects_to_stay
            ],
        )
