...
pyqt_plugin_path = os.path.join(os.path.dirname(PyQt5.__file__), "Qt",
                                "plugins", "platforms")
...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        "Folder where the meta information is stored. Default the same as input_folder."
    )
    parser.add_argument(
        "--edge_file",
        type=str,
        default=None,
        help=
        "Path to the scene graph relationship json file. If not provided, the edges will not be shown."
    )
    parser.add_argument("--sleep_time",
                        type=float,
                        default=0.1,
                        help="Time to sleep between each frame.")
    parser.add_argument("--follow_cam",
                        action="store_true",
                        help="If set, follow the camera pose.")
    parser.add_argument(
        "--use_original_color",
        action="store_true",
        help="If set, will use color scheme from the CFSLAM pipeline.")
    parser.add_argument(
        "--height_cutoff",
        type=float,
        default=np.inf,
        help="Object nodes above this height will not be shown.")
    return parser
...
def main(args):
    ...
    if not args.use_original_color:
        distinct_colors = distinctipy.get_colors(len(main.class_colors),
                                                 pastel_factor=0.5)
    ...
    while True:
        tic = time.time()
        camera_pose, objects, bg_objects = load_frame(
            frame_paths[main.frame_idx])
        ...
        if main.color_mode == COLOR_MODE.RGB:
            ...
            objects.color_by_most_common_classes(main.class_colors,
                                                 color_bbox=True)
            if bg_objects is not None:
                bg_objects.color_by_most_common_classes(main.class_colors,
                                                        color_bbox=True)
        ...
        if args.follow_cam:
            ...
            if main.show_cam and camera_pose is not None:
                camera_frustum = better_camera_frustum(camera_pose,
                                                       680 // 2,
                                                       1200 // 2,
                                                       scale=0.2,
                                                       color=[0, 1., 0])
