...
def main(args):
    ...
    video_save_path = Path(
        args.input_folder).parent / f"{cfg.gsa_variant}_{cfg.save_suffix}.mp4"

    # Save each video separately in this folder
    sep_video_save_folder = Path(
        args.input_folder
    ).parent.parent / "anime_all_frames" / f"{cfg.gsa_variant}_{cfg.save_suffix}"
    ...
    for i in trange(len(frame_paths)):
        ...
        if color_mode_instance:
            ...
            if bg_objects is not None:
                bg_objects.color_by_most_common_classes(class_colors,
                                                        color_bbox=True)
        ...
        result_frames_sep["render_rgb"].append(
            (render_rgb * 255).astype(np.uint8))
        result_frames_sep["render_class"].append(
            (render_class * 255).astype(np.uint8))
