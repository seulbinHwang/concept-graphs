...
def load_result(result_path):
    ...
    if potential_path != result_path:
        print(
            f"Resolved symlink for result_path: {result_path} -> \n{potential_path}"
        )
...
def main(args):
    ...
    if args.edge_file is not None:
        ...
        for edge in edges:
            ...
            line_mesh = LineMesh(points=np.array(
                [obj_centers[id1], obj_centers[id2]]),
                                 lines=np.array([[0, 1]]),
                                 colors=[1, 0, 0],
                                 radius=0.02)
    ...
    def color_by_clip_sim(vis):
        ...
        similarities = F.cosine_similarity(text_query_ft.unsqueeze(0),
                                           objects_clip_fts,
                                           dim=-1)
        max_value = similarities.max()
        min_value = similarities.min()
        normalized_similarities = (similarities - min_value) / (max_value -
                                                                min_value)
        probs = F.softmax(similarities, dim=0)
        max_prob_idx = torch.argmax(probs)
        similarity_colors = cmap(
            normalized_similarities.detach().cpu().numpy())[..., :3]
        ...
        for i in range(len(objects)):
            ...
            pcd.colors = o3d.utility.Vector3dVector(
                np.tile([
                    ...
                    similarity_colors[i, 1].item(), similarity_colors[i,
                                                                      2].item()
                ], (len(pcd.points), 1)))
