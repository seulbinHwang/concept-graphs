...
try:
    LLAVA_CKPT_PATH = os.environ["LLAVA_CKPT_PATH"]
except KeyError:
    raise ValueError(
        "Please set the environment variable LLAVA_CKPT_PATH to the path of the LLaVA checkpoint"
    )
...
if __name__ == "__main__":
    ...
    global_pcd.points = o3d.utility.Vector3dVector(
        np.concatenate([pcd.points for pcd in pcds]))
    global_pcd.colors = o3d.utility.Vector3dVector(
        np.concatenate([pcd.colors for pcd in pcds]))
    object_indices = np.concatenate(
        [np.full(len(pcd.points), i) for i, pcd in enumerate(pcds)])
    ...
    while True:
        ...
        for i, idx_det in enumerate(idx_sort[:5]):
            ...
            image_feature = chat.encode_image(image_tensor[None,
                                                           ...].half().cuda())
        ...
        print(
            f"Using the mean feature from {len(crop_features)} image crops...")
