...
def get_parser():
    ...
    parser.add_argument("--replica_semantic_root",
                        type=Path,
...
def eval_replica(
    ...
    gt_pc_path = os.path.join(args.replica_semantic_root, scene_id_,
                              "Sequence_1", "saved-maps-gt")
    gt_pose_path = os.path.join(args.replica_semantic_root, scene_id_,
    ...
    result_paths = glob.glob(
        os.path.join(args.replica_root, scene_id, "pcd_saves",
                     f"full_pcd_{args.pred_exp_name}*.pkl.gz"))
    if len(result_paths) == 0:
        raise ValueError(
            f"No result found for {scene_id} with {args.pred_exp_name}")
    ...
    object_feats = object_feats / object_feats.norm(
        dim=-1, keepdim=True)  # (num_objects, D)
    ...
    if args.n_exclude == 1:
        if results['bg_objects'] is None:
            print(
                "Warning: no background objects found. This is expected if only SAM is used, but not the detector. "
            )
        else:
            ...
            for obj in bg_objects:
                ...
                object_class = torch.cat(
                    [object_class, object_class.new_full([1], c)])
...
def main(args: argparse.Namespace):
    ...
    if args.n_exclude == 1:
        exclude_class = [class_names.index(c) for c in ["other"]]
    elif args.n_exclude == 4:
        exclude_class = [
            class_names.index(c) for c in ["other", "floor", "wall", "ceiling"]
        ]
    elif args.n_exclude == 6:
        exclude_class = [
            class_names.index(c)
            for c in ["other", "floor", "wall", "ceiling", "door", "window"]
        ]
    else:
    ...
    save_path = "./results/%s/replica_ex%d_results.csv" % (args.pred_exp_name,
                                                           args.n_exclude)
