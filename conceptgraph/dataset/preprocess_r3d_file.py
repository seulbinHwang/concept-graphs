...
def load_conf(filepath):
    with open(filepath, 'rb') as conf_fh:
        ...
        conf_img = resize_depth(
            conf_img, desired_width,
            desired_height)  # Using the same resizing function as depth
    return conf_img
...
def resize_depth(depth_img, desired_width, desired_height):
    return cv2.resize(depth_img, (desired_width, desired_height),
...
def main():
    ...
    if args.output_dir is None:
        ...
        args.output_dir = str(datapath.parent /
                              (datapath.name + "_preprocessed"))

    print(
        f"Preprocessing Record3D data from \n{args.datapath} to \n{args.output_dir}"
    )
    ...
    intrinsics_dict = adjust_intrinsics(intrinsics_dict, original_width,
                                        original_height)
    ...
    for i in trange(len(color_paths)):
        ...
        high_conf_depth = np.where(high_conf_mask, depth,
                                   0)  # Set depth to 0 where conf is not 2
        ...
        write_color(os.path.join(args.output_dir, "rgb", basename + ".png"),
                    color)
        # depth_path = os.path.splitext(os.path.basename(depth_paths[i]))[0] + ".png"
        write_depth(os.path.join(args.output_dir, "depth", basename + ".png"),
                    depth)
        # conf_path = os.path.splitext(os.path.basename(conf_paths[i]))[0] + ".npy"
        write_conf(os.path.join(args.output_dir, "conf", basename + ".npy"),
                   conf)
        write_conf_img(
            os.path.join(args.output_dir, "conf_images", basename + ".png"),
            conf)
        write_pose(os.path.join(args.output_dir, "poses", basename + ".npy"),
                   poses[i])
        write_depth(
            os.path.join(args.output_dir, "high_conf_depth", basename + ".png"),
            high_conf_depth)
